...
var	default_elements = require('./elements');
var default_rules = require('./rules');

var grammar = fs.readFileSync(__dirname + '/html5.pegjs', "UTF-8"),
	parser = peg.buildParser(grammar, { cache: true });

module.exports.lint = function Lint(src, rules, elements) {
	var info = [];

	elements = elements || {};
	_.defaults(elements, default_elements);
		...
		parser_emit: function(msg, match, offset) {
			...
			switch(msg) {
				...
				case "TAG_OPEN" :
					...
					if( elements[ tag ] && elements[ tag ].isVoid ) {
						stack.pop();
						cxt = cxt.parentNode;
					}
					break;
				...
				case "ATTRIBUTE_NAME" :
					...
					if( !(
						(elements["*"] && elements["*"].attributes && elements["*"].attributes[attr]) ||
						(elements[tag] && elements[tag].attributes && elements[tag].attributes[attr]) ||
						/^data-.+$/.test(attr)
					  )	 ) 
					{
