...
var read_tag_open = function tag_open(str, flags, rules) {
	var re = /^([<][/]?)\s*([-A-Za-z0-9_]+)/,
		match = re.exec(str);
...
var read_attribute_space = function attr_space(str, flags, rules) {
	var r = read_white(str, flags, rules);
...
var Lint = exports.Lint = function HTMLLint(str, flags, rules) {
	...
	while(remaining) {
		...
		if(options.length === 0) {
			options = [read_white, read_tag_open, read_comment, read_chars];
		}
	}
