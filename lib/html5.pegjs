...
attribute_name
	...
	/ namespace:(unquoted_name ":")? name:unquoted_name {
		if(namespace.length) {
			warn('ATTRIBUTE_WITH_NAMESPACE', namespace, offset, line, column);
		}
		emit("ATTRIBUTE_NAME", (namespace || []).concat(name), offset, line, column);
	}
	/ f:(unquoted_name ":") {
		warn("BAD_ATTRIBUTE_NAME_TRAILING_COLON", f, offset, line, column);
	}
	/ ":" {
		warn("BAD_ATTRIBUTE_NAME_COLON", null, offset, line, column);
	}
